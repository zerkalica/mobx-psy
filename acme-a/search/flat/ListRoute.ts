...
import { AcmeRouterCodec, AcmeRouterCodecError, AcmeRouterCodecTag, NullablePartial } from '@acme/router/codec'
import { AcmeRouterQueryMap } from '@acme/router/queryMap'
import { AcmeRouterSegMap } from '@acme/router/segMap'
import { psyObjectMerge } from '@psy/psy/object/merge'
import { psyObjectSetter } from '@psy/psy/object/setter'

const priceMaxSegMap = new AcmeRouterSegMap({
  '1': [1e6],
  '2': [2e6],
  '3': [3e6],
  '4': [4e6],
...
const parkingSegMap = new AcmeRouterSegMap({
  's-parkovkoi': ['OPEN', 'UNDERGROUND'],
  's-podzemnoi-parkovkoi': ['UNDERGROUND'],
} as const)

const parkingMap = new AcmeRouterQueryMap({
  OPEN: 'OPEN',
  UNDERGROUND: 'UNDERGROUND',
} as const)

const dealSegMap = new AcmeRouterSegMap({
  pokupka: ['SELL'],
  arenda: ['RENT'],
} as const)

const dealMap = new AcmeRouterQueryMap({
  SELL: 'SELL',
  RENT: 'RENT',
} as const)

const realtySegMap = new AcmeRouterSegMap({
  kvartiri: ['FLAT'],
  komnati: ['ROOM'],
  doma: ['HOUSE'],
  uchastka: ['LAND'],
...
const realtyMap = new AcmeRouterQueryMap({
  FLAT: 'FLAT',
  ROOM: 'ROOM',
  HOUSE: 'HOUSE',
  LAND: 'LAND',
...
export class AcmeSearchFlatListRoute {
  ...
  protected static codec = new AcmeRouterCodec(AcmeSearchFlatListRoute.regExSrc.bind(AcmeSearchFlatListRoute))

  protected static regExSrc(
    t: AcmeRouterCodecTag,
    p: Record<
  ...
  @computed.struct get get() {
    const seg = this.seg
    if (!seg) throw new AcmeRouterCodecError(`URL '${this.loc.url ?? '/'}' not matched ${this}`)
