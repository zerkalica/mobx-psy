import { psyMockPromisify } from '@psy/psy/mock/promisify'

import { AcmeSearchFlatDTO } from '../../flat/model'
...
function createFlatsMock() {
  const flatsAll: AcmeSearchFlatDTO[] = []
  ...
  return (filter?: Partial<AcmeSearchFlatDTO & { page: number }>) => {
    const items = filter
...
export function acmeSearchBootDevMocks({ errorRate, timeout }: { errorRate: number; timeout: number }) {
