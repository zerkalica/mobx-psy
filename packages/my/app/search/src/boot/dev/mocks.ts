...
import { MySearchFlatDto } from '../../flat'
...
function createFlatsMock() {
  const flatsAll: MySearchFlatDto[] = []
  ...
  return (filter?: Partial<MySearchFlatDto & { page: number }>) => {
    const items = filter
...
export function mySearchBootDevMocks({ errorRate, timeout }: { errorRate: number; timeout: number }): MyFetch {
