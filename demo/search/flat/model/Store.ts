...
import { DemoLibFetchSync, useDemoLibFetchContext } from '@demo/lib-fetch'
import { DemoLibRouterLocation } from '@demo/lib-router'
import React from 'react'

import { DemoSearchFlatModelFilter } from './Filter'
...
export class DemoSearchFlatModel {
  ...
  constructor(dto: DemoSearchFlatDto) {
    this.id = dto.id
...
export class DemoSearchFlatModelStore {
  ...
  static use() {
    const { fetch, location } = useDemoLibFetchContext()

    return React.useMemo(() => new this(location, fetch), [fetch, location])
  }

  @computed get filter() {
    return new DemoSearchFlatModelFilter(this.location)
  }
  ...
  protected normalize(raw: DemoSearchFlatDto): DemoSearchFlatModel {
